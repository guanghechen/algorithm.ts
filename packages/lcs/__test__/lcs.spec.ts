...
import { lcs_dp, lcs_myers, lcs_myers_linear_space } from '../src'
import { lcs_size_dp, lcs_size_myers, lcs_size_myers_linear_space } from '../src'
...
function test_lcs_size(lcs_size: ILcsSize) {
...
function test_lcs(lcs: ILcs) {
  const test_case = <T extends string | unknown[]>(s1: T, s2: T, expected: number): void => {
    ...
    let k: number = 0
    for (let i: number = 0; i < s1.length && k < actual.length; ++i) if (s1[i] === actual[k]) ++k
    expect(k).toEqual(actual.length)

    k = 0
    for (let i: number = 0; i < s2.length && k < actual.length; ++i) if (s2[i] === actual[k]) ++k
    expect(k).toEqual(actual.length)
  }
...
function test_lcs_minimal_lexicographical(lcs: ILcs) {
  const test_case = <T extends string | unknown[]>(
    s1: T,
    s2: T,
    expected: [number, number][],
  ): void => {
    const actual: [number, number][] = lcs(s1.length, s2.length, (x, y) => s1[x] === s2[y])
    expect(actual).toEqual(expected)
  }
