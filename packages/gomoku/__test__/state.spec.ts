...
import { GomokuContext, GomokuState, createScoreMap, gomokuDirectionTypes } from '../src'
...
class TestHelper extends GomokuState {
  ...
  public $getCandidates(nextPlayer: number, scoreForPlayer: number): IGomokuCandidateState[] {
    const candidates: IGomokuCandidateState[] = this.expand(nextPlayer)
    const prevScore = this.score(nextPlayer, scoreForPlayer)
    for (const candidate of candidates) {
      this.forward(candidate.id, nextPlayer)
      candidate.score =
        nextPlayer === scoreForPlayer
          ? this.score(nextPlayer, scoreForPlayer) - prevScore
          : prevScore - this.score(nextPlayer, scoreForPlayer)
      this.revert(candidate.id)
    }
    return candidates
  }
}
...
describe('15x15', function () {
  ...
  const checkCandidates = (nextPlayer: number, scoreForPlayer: number): void => {
    expect(tester.expand(nextPlayer).sort(compareCandidate)).toEqual(
      tester.$getCandidates(nextPlayer, scoreForPlayer).sort(compareCandidate),
    )
  }
  ...
  test('candidate ids', async function () {
    ...
    for (const { filepath, title } of filepaths) {
      tester.init([])
      expect([title, getCandidateIds(0, 0)]).toEqual([title, [112]])
      expect([title, getCandidateIds(0, 1)]).toEqual([title, [112]])
      expect([title, getCandidateIds(1, 0)]).toEqual([title, [112]])
      expect([title, getCandidateIds(1, 1)]).toEqual([title, [112]])
      const pieces = await fs.readJSON(filepath)
      for (const { r, c, p } of pieces) {
        const id: number = tester.context.idx(r, c)
        tester.forward(id, p)
        const candidateIds: number[] = tester.$getCandidateIds()
        expect([title, getCandidateIds(0, 0)]).toEqual([title, candidateIds])
        expect([title, getCandidateIds(0, 1)]).toEqual([title, candidateIds])
        expect([title, getCandidateIds(1, 0)]).toEqual([title, candidateIds])
        expect([title, getCandidateIds(1, 1)]).toEqual([title, candidateIds])
      }
    }
  })
  ...
  test('candidates -- init all', async function () {
    tester.init([])
    expect(tester.expand(0)).toEqual([{ id: tester.context.MIDDLE_POS, score: 32 }])
    expect(tester.expand(1)).toEqual([{ id: tester.context.MIDDLE_POS, score: 32 }])
